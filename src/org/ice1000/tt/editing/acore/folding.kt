...
import org.ice1000.tt.editing.CAP_PI
import org.ice1000.tt.editing.CAP_SIGMA
import org.ice1000.tt.editing.LAMBDA
...
class ACoreFoldingBuilder : FoldingBuilderEx(), DumbAware {
	override fun getPlaceholderText(node: ASTNode) = when {
		node.elementType == ACoreTypes.LAMBDA -> LAMBDA
		node.elementType == ACoreTypes.PI -> CAP_PI
		node.elementType == ACoreTypes.SIGMA -> CAP_SIGMA
		else -> "..."
	}
	...
	private class FoldingVisitor(
		...
		private val document: Document) : ACoreVisitor() {
		private companion object {
			private val types = listOf(ACoreTypes.LAMBDA, ACoreTypes.PI, ACoreTypes.SIGMA)
		}
